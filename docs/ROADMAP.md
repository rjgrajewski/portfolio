...
> **Built on dev `2026-08-30`, then promoted to production the same day** (owner's call, after dev iteration). OQ-8 resolved and **empirically verified closed** (no credential path bypasses the breaker). OQ-4 resolved (bidirectional streaming isn't browser-reachable → sentence chunking). Voice EN = `Ruth`. Barge-in shipped (persistent hands-free session, adaptive echo guard). The **agent UI was rebuilt voice-first** — single-column portfolio + a fixed organic visualization + a hidden transcript, no chat column ([DECISIONS.md](DECISIONS.md); ARCHITECTURE.md §§ Product shape / Agentic UI pattern updated). Per-session cap fixed (fresh id per load, calmer copy, prod 40→150).
>
> **Prod cutover `2026-08-30`:** `portfolio-identity-prod` deployed for the first time (`portfolio-credentials-prod` Function URL + `portfolio-media-guest-prod` role, CORS locked to the prod origin, media breaker 120); `portfolio-api-prod` redeployed (guardrail-4 systemPrompt + shared breaker + `SESSION_MESSAGE_CAP` 150); `VITE_CREDENTIALS_URL` set on the `main` Amplify branch to the prod credentials Function URL; `dev`→`main` merged and built (Amplify job #5 green). Verified against `https://main.daz9bpic9q3nd.amplifyapp.com`: portfolio + orb render, text turn streams a grounded answer, orb tap enters voice mode and `runCapture` vends from the **prod** credentials Lambda (not dev, not the `disabled` app-level default) with Transcribe opening cleanly, no console errors. Still owner-only on a real phone: the acoustic barge-in loop and mobile Safari autoplay/keyboard behaviour (the browser pane can't grant a mic).
...
- [x] Scoped guest role — `portfolio-media-guest-<env>`, scoped to **exactly the browser's two calls**: `transcribe:StartStreamTranscriptionWebSocket` (the WebSocket form; the Node HTTP/2 `StartStreamTranscription` is a *separate* IAM action and deliberately not granted — this was the second staging failure) + `polly:SynthesizeSpeech`. Trust policy names only the vending Lambda. `identity-stack.ts` holds it + the Lambda. `npm run verify-oq8` (8/8) checks the browser action names via IAM policy simulation. Deployed to **dev and prod** (`portfolio-media-guest-prod`, `portfolio-credentials-prod`).
...
- [x] **Barge-in** — voice input is a persistent hands-free session (`stt.ts` `createVoiceSession`), not push-to-talk. The visitor can speak at any moment, including while the agent talks: the agent stops instantly (`player.stop()` + stream abort), the partial answer stays in the transcript flagged `interrupted`, and the utterance becomes the next turn. Self-interruption guard: `echoCancellation` + an adaptive echo-tracking threshold + a 300 ms sustain + a **700 ms disarm window after every playback start**. VAD logic unit-tested (`npm run verify-vad`, 11/11). Typing during speech + text-send-interrupts still work. *Acoustic loop on a real phone is the outstanding check.*
...
- [x] **On live staging + production:** dev and prod both built and serving the voice UI; text turn + voice-mode entry + prod credential vend + Transcribe open verified against the prod URL. *(Owner-only remaining: the full spoken round-trip and the acoustic barge-in loop on a physical phone.)*

**Exit:** full voice round-trip on desktop Chrome/Safari and mobile Safari/Chrome, with text still available at any point. **Code + dev + prod infra done; text path and voice-session/credential path verified live on prod; the on-phone spoken round-trip is the owner's remaining check.**
