...
- [x] Repo scaffold: npm workspaces (`frontend`, `backend/infra`), `frontend/` (Vite + React + TS + Tailwind, builds clean), `backend/infra/` (see below), `content/` (stub `README.md`, seeded from Phase 2), `scripts/`, `.nvmrc`, `.gitignore`, CI stubs (below). Local git repo initialized with `main` and `dev` branches (not yet pushed to a remote — see the Amplify line below).
...
- [ ] Amplify Hosting app connected to the repo; `dev` → staging URL, `main` → production URL, both building a placeholder page. **Blocked, not attempted via CLI**: connecting Amplify to a repo needs either GitHub's browser-only OAuth consent (the standard GitHub App flow — no CLI/API equivalent) or a personal access token passed to `aws amplify create-app --oauth-token`, and entering a token like that is off the table regardless of who provides it. There is also no GitHub remote yet — this repo is local-only so far. **Next steps, by hand:** (1) push this repo to GitHub with `main` and `dev` branches; (2) AWS Console → Amplify (region **EU (Frankfurt) / eu-central-1**, top-right region switcher) → **Create app** → **Host web app** → choose **GitHub** → authorize the "AWS Amplify" GitHub App for this repo (first-time browser consent) → select the repo and the `main` branch → Amplify should auto-detect `amplify.yml` at the repo root; confirm the build settings show it, then **Save and deploy** — this creates the `main` (production) URL; (3) in the new app, **Branches** tab → **Connect branch** → select `dev` from the same repo → connect — this creates the staging URL.
...
**Exit:** money can't leak silently (done — manual budgets + circuit-breaker design); a placeholder deploys to two URLs from two branches (**not yet** — blocked on the Amplify/GitHub step above).

---

## Phase 1 — Frontend shell & manual portfolio (the non-AI fallback)

- [ ] Design pass: type scale, colour, spacing, motion language — **timeboxed** (a few hours, not open-ended); establishes the visual language and must be presentable, but does not need to fully clear the "not templated / design-forward" bar yet — that bar is formally owned by [Phase 9](#phase-9--polish--responsiveaccessibility-pass) and is not required for Tuesday (see [README Status](../README.md#status))
- [ ] Landing view: photo + short blurb (placeholder copy), visually nudging toward the AI chat
- [ ] Section registry (`frontend/src/content/sections.ts`) — the shared id/title/order list
- [ ] `SectionShell` + reveal-in-place animation (the one code path for "show section X")
- [ ] Manual section components: Education, Amazon, FlowJob, Rhymind, Portfolio-itself (placeholder content OK)
- [ ] CV download — real PDF in `frontend/public/cv/cv.pdf`, wired to a visible button
- [ ] Responsive: desktop-primary, mobile genuinely usable (layout only at this stage)
