...
- [x] Manually verified Bedrock, Polly generative (EN + `Ola`/`Ewa`), and Transcribe streaming (`en-US`/`en-GB`/`pl-PL`) all work in Frankfurt, by real calls (synthesis + a full Polly→Transcribe round trip) rather than `describe` checks — see [ARCHITECTURE.md § Region](ARCHITECTURE.md#region). The scripted version of this check is still open, below.
- [ ] `scripts/check-availability.ts` — codify the above manual verification into a repeatable script (Bedrock Haiku 4.5, Polly generative EN + `Ola`/`Ewa`, Transcribe streaming `en-*` + `pl-PL`, all in Frankfurt)
- [ ] `scripts/estimate-cost.ts` — token math for the expected question mix vs the 100 PLN ceiling; commit the numbers
- [ ] Repo scaffold: npm workspaces, `frontend/`, `backend/`, `content/`, `docs/` (already holds planning docs), `.nvmrc`, CI stubs
- [ ] CDK skeleton: `config.ts` with dev/prod params; empty `guardrails-stack` (SNS topic only — **not** the budgets, those stay manual) wired to SNS; `cdk synth` green in CI
- [ ] Amplify Hosting app connected to the repo; `dev` → staging URL, `main` → production URL, both building a placeholder page
- [ ] Confirm dev/prod separation is real: separate CDK stacks, separate S3 prefixes, separate DynamoDB table names

**Exit:** money can't leak silently; a placeholder deploys to two URLs from two branches.
