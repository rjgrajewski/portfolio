...
- **Voice is the primary entry; the whole screen is the portfolio.** The layout is a single scrolling column (mobile and desktop alike). The agent is present as a fixed, organic **visualization** at the bottom of the viewport — breathing at rest, reacting to mic/playback amplitude in conversation — and, while a conversation is live, a subtle **AI-mode frame** around the viewport that signals "the agent is driving the interface". It does **not** occupy a docked chat column. The bottom zone is exactly two things: the visualization and, above it, the **transcript — hidden by default** as a slim one-line bar, expandable to full history, capped so it never buries an open section (re-collapsed on mobile whenever a section opens). There is **no state caption and no "leave" button**: the four blob states are meant to read on sight (the state stays available to screen readers via an `aria-live` region), and **a tap on the orb ends voice mode** — the voice-assistant convention. Interrupting the agent is a separate gesture: you just start speaking (barge-in). This replaces the earlier desktop two-zone layout (a fixed agent column beside a scrolling content column) — recorded in `docs/DECISIONS.md`. Text input stays reachable behind a small "Type" toggle in the bar, deliberately *not* styled into the voice aesthetic pending a device-test decision on whether typing belongs inside voice mode or is an exit from it.
...
- **Single-page only.** Sections open as a modal over the hero. **No new tabs. No route changes.** State (conversation, audio, language) must survive every interaction.
...
- A section opens as a **modal card over the hero** (`SectionModal`) on every breakpoint — the same `activeSectionStore` state a list tap and the agent write. The page behind stays visible (dimmed); it is not a full-screen takeover and not an in-place accordion, so the agent does not need to scroll and mobile does not feel like a new page. The agent presence (visualization + AI-mode frame) sits **above** the modal — a section reveal is when it's most important to see the agent is driving the UI, not least. The stacking order is one named scale in `styles/index.css` (`--z-section-takeover` 40 < `--z-header` 45 < `--z-ai-frame` 50 < `--z-agent-dock` 60); header stays above the modal so the CV remains reachable; the dock is rendered through a portal to `document.body` so no ancestor can re-base its `position: fixed`.
...
4. **No candidacy assessment, no skill-gap claims.** The agent never says whether Rafal is a fit / match / "right person", never compares him to other candidates or to a seniority bar, never rates or scores him, never predicts how he "would do" — that verdict is the visitor's. And absence of a topic from the corpus is the agent's *knowledge boundary*, never a fact about Rafal's skills: it says "I don't have information about his experience with X" (not "he hasn't worked with X"), gives the nearest real work it *can* speak to, and refers the specifics to Rafal. Same family as grounding — a wrong answer here can cost an interview. Added 2026-08-30 after the agent told a recruiter Rafal was "probably not the right fit"; cases in [`prompt-injection-tests.md`](prompt-injection-tests.md#candidacy-assessment--skill-gap-claims).
...
│   │   │   │   ├── SectionShell.tsx
│   │   │   │   ├── SectionModal.tsx    # modal over the hero (every breakpoint)
